...
const createTransactionsDraft = (toIds: any[]) => {
  const transactionObj: { typeId: string; id: any }[] = []
...
const createTransactions = (fromId: any, toIds: any[]) =>
  execute({
...
// return a promise
const createState = (key: any, nameEN: any, nameDE: any, intialState: any) =>
  execute({
  ...
  export const importProductStates = () =>
    execute({
      ...
      .then((response) => response.body?.id) // Extract initialId from the response
      .then((initialId) =>
